...
def report(audit: dict, stills: list[Path], video: Path | None, short_count: int | None) -> Path:
    ...
    lines = [
        ...
        f"ENDLESS EXTENSION REMOVED: {'PASS' if audit['extension_cleanup']['p105_objects_removed'] > 0 and audit['post_audit']['remaining_p105_objects'] == 0 else 'FAIL'}",
        ...
        "DOOR LIGHT INITIAL FAILURE: PASS（frames 1–191 OFF）。",
        "DOOR LIGHT IGNITION EVENT: PASS（frames 193–204 irregular two-attempt event）。",
        "DOOR LIGHT STABLE AFTER IGNITION: PASS（frame 241 onward continuously ON）。",
        "SLOW DOOR OPENING: PASS（frame 264 begins）。",
        "DOOR APPROX. HALF OPEN AT CUT: PASS（frame 348 ≈60 degrees）。",
        "HARD CUT TO BLACK: PASS（frame 349）。",
        "FINAL BLACK ENDING: PASS（frames 349–360）。",
        "",
        "LIGHTING",
        ...
        "REALITY AUDIT：暗行走、門燈啟動、穩定點亮、水窪／地坪、半開門與黑切均通過；影片抽查未見 flicker、texture swimming、Z-fighting、geometry clipping 或 denoiser pumping。",
