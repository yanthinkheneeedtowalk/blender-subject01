#!/usr/bin/env python3
"""Fix pass: glass must read as glass, dust as edge film, cloth as fabric.
...
def enrich_photo_faint():
    """Blurred, unidentifiable figure under glass. No tear, skull, or mutation."""
    img = bpy.data.images.get("P2_PHOTO_BLUR")
    if img is None:
        img = bpy.data.images.new("P2_PHOTO_BLUR", 512, 640, alpha=False)
    ...
    for i in range(n):
        ...
        u = x / max(w - 1, 1)
        v = y / max(h - 1, 1)
        paper = 0.42 + 0.04 * math.sin(u * 9.1) * math.cos(v * 7.4)
        paper += rng.uniform(-0.018, 0.018)
        hx = (u - 0.50) / 0.16
        hy = (v - 0.64) / 0.20
        head = math.exp(-(hx * hx + hy * hy) * 1.15)
        sx = (u - 0.50) / 0.28
        sy = (v - 0.34) / 0.22
        sh = math.exp(-(sx * sx + sy * sy) * 0.85)
        fig = max(head * 0.55, sh * 0.38)
        # Chemical fade, not a readable face.
        fade = 0.78 + 0.22 * math.sin((u * 4.2 + v * 3.1) * math.pi)
        tone = paper - fig * 0.22 * fade
        vig = 1.0 - 0.28 * (((u - 0.5) * 1.6) ** 2 + ((v - 0.5) * 1.35) ** 2)
        tone = max(0.08, min(0.52, tone * vig))
        r = tone * 0.92
        g = tone * 0.78
        b = tone * 0.58
        o = i * 4
    ...
    if mat is None:
        mat = bpy.data.materials.new("MAT_P2_Photo")
    mat.use_nodes = True
    nt = mat.node_tree
    nt.nodes.clear()
    ...
    blur_n = nt.nodes.new("ShaderNodeTexNoise")
    blur_n.inputs["Scale"].default_value = 18.0
    blur_n.inputs["Detail"].default_value = 2.0
    mix = nt.nodes.new("ShaderNodeMix")
    mix.data_type = "RGBA"
    sock(mix, "Factor_Float").default_value = 0.18
    sock(mix, "B_Color").default_value = (0.28, 0.22, 0.16, 1)
    bsdf.inputs["Roughness"].default_value = 0.94
    if "Specular IOR Level" in bsdf.inputs:
        bsdf.inputs["Specular IOR Level"].default_value = 0.08
    nt.links.new(tex.outputs["Color"], sock(mix, "A_Color"))
    nt.links.new(blur_n.outputs["Fac"], sock(mix, "Factor_Float"))
    nt.links.new(sock(mix, "Result_Color"), bsdf.inputs["Base Color"])
    nt.links.new(bsdf.outputs["BSDF"], out.inputs["Surface"])
    photo = bpy.data.objects.get("P2_FRAME_PHOTO")
    if photo:
        photo.data.materials.clear()
        photo.data.materials.append(mat)


def rebuild_glass():
    """Transparent glass with hairline scratches and fingerprint roughness. No marble veins."""
    mat = bpy.data.materials["MAT_P2_Glass"]
    mat.use_nodes = True
    set_blend(mat, "BLEND")
    if hasattr(mat, "refraction_depth"):
        try:
            mat.refraction_depth = 0.003
        except TypeError:
            pass
    nt = mat.node_tree
    nt.nodes.clear()
    ...
    trans.inputs["Color"].default_value = (0.93, 0.94, 0.95, 1)
    ...
    mix_fac = nt.nodes.new("ShaderNodeMath")
    mix_fac.operation = "MULTIPLY"
    mix_fac.inputs[1].default_value = 0.55
    mix_add = nt.nodes.new("ShaderNodeMath")
    mix_add.operation = "ADD"
    mix_add.inputs[1].default_value = 0.08
    mixs = nt.nodes.new("ShaderNodeMixShader")
    coord = nt.nodes.new("ShaderNodeTexCoord")
    # Hairline scratches: long, sparse, low distortion.
    mapn = nt.nodes.new("ShaderNodeMapping")
    mapn.inputs["Scale"].default_value = (1.0, 0.12, 1.0)
    mapn.inputs["Rotation"].default_value = (0.0, 0.0, 0.31)
    ...
    sc.color_ramp.interpolation = "CONSTANT"
    sc.color_ramp.elements[0].position = 0.0
    sc.color_ramp.elements[0].color = (0.0, 0.0, 0.0, 1)
    sc.color_ramp.elements[1].position = 0.93
    sc.color_ramp.elements[1].color = (1, 1, 1, 1)
    # Fingerprints / smudges — roughness only.
    nfp = nt.nodes.new("ShaderNodeTexNoise")
    nfp.inputs["Scale"].default_value = 9.5
    nfp.inputs["Detail"].default_value = 8.0
    if "Roughness" in nfp.inputs:
        nfp.inputs["Roughness"].default_value = 0.45
    fp = nt.nodes.new("ShaderNodeValToRGB")
    fp.color_ramp.elements[0].position = 0.68
    ...
    mul_s.inputs[1].default_value = 0.28
    mul_f = nt.nodes.new("ShaderNodeMath")
    mul_f.operation = "MULTIPLY"
    mul_f.inputs[1].default_value = 0.22
    add = nt.nodes.new("ShaderNodeMath")
    add.operation = "ADD"
    base = nt.nodes.new("ShaderNodeMath")
    base.operation = "ADD"
    base.inputs[1].default_value = 0.028
    ...
    nt.links.new(add.outputs["Value"], base.inputs[0])
    nt.links.new(base.outputs["Value"], gloss.inputs["Roughness"])
    nt.links.new(mul_s.outputs["Value"], bump.inputs["Height"])
    nt.links.new(bump.outputs["Normal"], gloss.inputs["Normal"])
    nt.links.new(lw.outputs["Facing"], mix_fac.inputs[0])
    nt.links.new(mix_fac.outputs["Value"], mix_add.inputs[0])
    nt.links.new(trans.outputs["BSDF"], mixs.inputs[1])
    nt.links.new(gloss.outputs["BSDF"], mixs.inputs[2])
    nt.links.new(mix_add.outputs["Value"], mixs.inputs["Fac"])
...
def rebuild_dust():
    """Corner/edge film with fine specks. Center stays thin so the photo reads."""
    ...
    px.operation = "POWER"
    px.inputs[1].default_value = 3.2
    pz = nt.nodes.new("ShaderNodeMath")
    pz.operation = "POWER"
    pz.inputs[1].default_value = 3.2
    add = nt.nodes.new("ShaderNodeMath")
    add.operation = "ADD"
    cr = nt.nodes.new("ShaderNodeValToRGB")
    cr.color_ramp.elements[0].position = 0.00
    cr.color_ramp.elements[0].color = (0.34, 0.31, 0.25, 0.03)
    mid = cr.color_ramp.elements.new(0.28)
    mid.color = (0.35, 0.32, 0.25, 0.14)
    edge = cr.color_ramp.elements.new(0.55)
    edge.color = (0.38, 0.34, 0.27, 0.52)
    cr.color_ramp.elements[1].position = 0.88
    cr.color_ramp.elements[1].color = (0.42, 0.37, 0.29, 0.90)
    speck = nt.nodes.new("ShaderNodeTexNoise")
    speck.inputs["Scale"].default_value = 160.0
    speck.inputs["Detail"].default_value = 8.0
    sm = nt.nodes.new("ShaderNodeMapRange")
    sm.inputs["To Min"].default_value = 0.78
    sm.inputs["To Max"].default_value = 1.12
    mul = nt.nodes.new("ShaderNodeMath")
    mul.operation = "MULTIPLY"
    clamp = nt.nodes.new("ShaderNodeClamp")
    clamp.inputs["Min"].default_value = 0.02
    clamp.inputs["Max"].default_value = 0.90
    colmix = nt.nodes.new("ShaderNodeMix")
    colmix.data_type = "RGBA"
    sock(colmix, "A_Color").default_value = (0.40, 0.36, 0.29, 1)
    sock(colmix, "B_Color").default_value = (0.26, 0.23, 0.18, 1)
    bump = nt.nodes.new("ShaderNodeBump")
    ...
    if dust and dust.type == "MESH":
        ...
        # Keep independent dust mesh; puff corners only, no clip into glass.
        rng = random.Random(77)
        for v in dust.data.vertices:
            nx = abs(v.co.x) / 0.084
            nz = abs(v.co.z) / 0.109
            corner = min(1.0, (nx ** 3.0 + nz ** 3.0) * 0.65)
            v.co.y = -0.00045 - corner * 0.0016 - rng.uniform(0.0, 0.00012)
        dust.location.y = -0.0128


def rebuild_cloth():
    """Pale worn rag: visible weave, stains, dust, sheen. Not clay, not rubber."""
    ...
    nt.links.new(bsdf.outputs["BSDF"], out.inputs["Surface"])
    if "Specular IOR Level" in bsdf.inputs:
        bsdf.inputs["Specular IOR Level"].default_value = 0.025
    if "Sheen Weight" in bsdf.inputs:
        ...
        if "Sheen Tint" in bsdf.inputs:
            try:
                bsdf.inputs["Sheen Tint"].default_value = (0.55, 0.50, 0.42, 1)
            except TypeError:
                pass
    if "Subsurface Weight" in bsdf.inputs:
        ...
        if "Subsurface Radius" in bsdf.inputs:
            try:
                bsdf.inputs["Subsurface Radius"].default_value = (0.6, 0.2, 0.1)
            except TypeError:
                pass
    coord = nt.nodes.new("ShaderNodeTexCoord")
    # ~8 mm thread bundles — readable from sit and macro.
    map1 = nt.nodes.new("ShaderNodeMapping")
    map1.inputs["Scale"].default_value = (1.0, 1.0, 1.0)
    w1 = nt.nodes.new("ShaderNodeTexWave")
    w1.wave_type = "BANDS"
    w1.inputs["Scale"].default_value = 95.0
    w1.inputs["Distortion"].default_value = 0.35
    ...
    w2.wave_type = "BANDS"
    w2.inputs["Scale"].default_value = 95.0
    w2.inputs["Distortion"].default_value = 0.35
    ...
    sock(weave_col, "Factor_Float").default_value = 0.28
    sock(weave_col, "B_Color").default_value = (0.62, 0.60, 0.55, 1)
    stain_n = nt.nodes.new("ShaderNodeTexNoise")
    stain_n.inputs["Scale"].default_value = 6.5
    stain_n.inputs["Detail"].default_value = 10.0
    cr = nt.nodes.new("ShaderNodeValToRGB")
    cr.color_ramp.elements[0].position = 0.20
    cr.color_ramp.elements[0].color = (0.16, 0.13, 0.10, 1)
    s1 = cr.color_ramp.elements.new(0.48)
    s1.color = (0.26, 0.23, 0.18, 1)
    cr.color_ramp.elements[1].position = 0.84
    cr.color_ramp.elements[1].color = (0.38, 0.34, 0.28, 1)
    ...
    mixd.data_type = "RGBA"
    mixd.blend_type = "MIX"
    sock(mixd, "B_Color").default_value = (0.30, 0.27, 0.21, 1)
    ...
    mixf.data_type = "RGBA"
    sock(mixf, "B_Color").default_value = (0.44, 0.40, 0.33, 1)
    rough = nt.nodes.new("ShaderNodeValToRGB")
    rough.color_ramp.elements[0].position = 0.22
    rough.color_ramp.elements[0].color = (0.68, 0.68, 0.68, 1)
    ...
    bump_w.inputs["Strength"].default_value = 0.55
    bump_w.inputs["Distance"].default_value = 0.0009
    bump_s = nt.nodes.new("ShaderNodeBump")
    bump_s.inputs["Strength"].default_value = 0.18
    ...
    sock(weave_col, "Factor_Float").default_value = 0.32
    nt.links.new(weave.outputs["Value"], sock(weave_col, "Factor_Float"))
